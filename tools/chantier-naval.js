...
function cleanupShip(ship) {
  ...
  ['origineExotique', 'lestInverse', 'malusHauturier'].forEach(key => {
    if (!out[key]) delete out[key];
  });
...
function renderOutput() {
  ...
  exportEl.hidden = !state.exportOpen;
  toggleExportBtn.textContent = state.exportOpen ? 'Masquer export' : 'Afficher export';
  toggleExportBtn.setAttribute('aria-expanded', String(state.exportOpen));
}
