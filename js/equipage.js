...
function buildStatBar(val) {
  ...
  if (val <= MAX_NORMAL) {
    ...
    // Le point de transition suit la longueur de la barre
    // À val=1 : transition à 20% (quasi tout bleu)
    // À val=5 : transition à 80% (large section or)
    transitionPct = Math.round((val / MAX_NORMAL) * 80);
  } else {
    // 6+ : barre pleine, transition recule vers la gauche
    revealPct     = 100;
    const excess  = Math.min(val - MAX_NORMAL, 4); // plafonné à +4
    transitionPct = Math.max(5, 80 - excess * 18); // recule de 18% par point
  }

  const gradient = `linear-gradient(90deg,
    #1a3a52 0%,
    var(--sea-light) ${transitionPct * 0.6}%,
    #9a7a28 ${transitionPct}%,
    var(--gold) ${Math.min(transitionPct + 20, 100)}%
  )`;

  return `
    <div class="stat-bar">
      <div class="stat-bar-reveal" data-target="${revealPct}">
        <div class="stat-bar-fill" style="background:${gradient}"></div>
      </div>
    </div>`;
}
    ...
    if (grid && lines.length >= 1) {
      ...
      if (vals.length >= 6 && !vals.slice(0,6).some(isNaN)) {
        ...
        requestAnimationFrame(() => requestAnimationFrame(() => {
          grid.querySelectorAll('.stat-bar-reveal').forEach(b => { b.style.width = b.dataset.target + '%'; });
        }));
      } else {
