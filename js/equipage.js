...
function buildStatBar(val) {
  ...
  if (val <= MAX_NORMAL) {
    // Barre révélatrice proportionnelle à val/5
    // Point de transition fixe à 50% — on voit d'autant plus d'or
    // que la barre est longue
    revealPct     = Math.round((val / MAX_NORMAL) * 100);
    transitionPct = 50;
  } else {
    // 6+ : barre pleine, point de transition recule vers la gauche
    // de 10% par point au-delà de 5 (plafonné à 0%)
    revealPct     = 100;
