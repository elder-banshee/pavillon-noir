...
function buildStatBar(val) {
  ...
  return `
    <div class="stat-bar" style="background:${gradient}">
      <div class="stat-bar-reveal" data-target="${revealPct}" style="width:100%"></div>
    </div>`;
}
    ...
    if (grid && lines.length >= 1) {
      ...
      if (vals.length >= 6 && !vals.slice(0,6).some(isNaN)) {
        ...
        requestAnimationFrame(() => requestAnimationFrame(() => {
          const reveals = grid.querySelectorAll('.stat-bar-reveal');
          console.log('RAF déclenché, reveals trouvés :', reveals.length);
          reveals.forEach(b => {
            const target = parseInt(b.dataset.target);
            const newWidth = (100 - target) + '%';
            console.log('target:', target, '→ width:', newWidth);
