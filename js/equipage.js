...
function buildStatBar(val) {
  const MAX_NORMAL = 5;
  let revealPct, bgPosition;

  if (val <= MAX_NORMAL) {
    // Cas normal : on révèle val/5 de la barre, gradient fixe
    revealPct  = Math.round((val / MAX_NORMAL) * 100);
    bgPosition = '0% 0%';
  } else {
    // Valeur légendaire 6+ : barre pleine, gradient décalé vers la droite
    // Chaque point au-delà de 5 "chasse" le bleu de 15%
    revealPct  = 100;
    const shift = Math.min((val - MAX_NORMAL) * 15, 50); // max 50% de décalage
    bgPosition = shift + '% 0%';
  }
  ...
  return `
    <div class="stat-bar">
      <div class="stat-bar-reveal" data-target="${revealPct}">
        <div class="stat-bar-fill" style="background-position:${bgPosition}"></div>
      </div>
    </div>`;
}
...
async function loadAll() {
  ...
  try {
    ...
    if (crewTable && lines.length >= 2) {
      ...
      // Animer les barres de composition
      requestAnimationFrame(() => {
